...
const loadSettings = () => {
    const fallback = { muted: false, volume: 0.7 };
...
export class SfxManager {
    ...
    shouldRateLimit(key, minIntervalMs) {
    ...
    playTone(config) {
        ...
        oscillator.frequency.exponentialRampToValueAtTime(Math.max(1, config.endFrequency), context.currentTime + config.duration);
