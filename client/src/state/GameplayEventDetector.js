...
const clonePlayers = (players) => new Map(Array.from(players.entries(), ([id, player]) => [id, { ...player, stats: { ...player.stats } }]));
const cloneBullets = (bullets) => new Map(Array.from(bullets.entries(), ([id, bullet]) => [id, { ...bullet }]));
const cloneZones = (zones) => new Map(Array.from(zones.entries(), ([id, zone]) => [id, { ...zone }]));
export class GameplayEventDetector {
    previousPlayers = new Map();
    previousBullets = new Map();
    previousZones = new Map();
    initialized = false;
    consume(next) {
        if (!this.initialized) {
...
//# sourceMappingURL=GameplayEventDetector.js.map