import { AbilityId, AbilityVfxPhase, PLAYER_RADIUS } from "@tankes/shared";
const trim = (arr, max) => {
    if (arr.length > max)
        arr.splice(0, arr.length - max);
};
function handleDash(cue, fx) {
...
function handleReactiveShield(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handlePiercingBurst(cue, fx) {
    if (cue.phase !== AbilityVfxPhase.Cast)
        return;
...
function handleProximityMine(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handleLightTurret(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handleTankRepair(cue, fx) {
    if (cue.phase !== AbilityVfxPhase.Cast)
        return;
...
function handleTacticalFog(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handleOverheat(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handleOrbitalBarrage(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handleSiegeMode(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
...
function handleHomingMissile(cue, fx) {
    if (cue.phase === AbilityVfxPhase.Cast) {
